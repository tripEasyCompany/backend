...
const collectionController = {
  ...
  async get_collection(req, res, next) {
    ...
    if (
      isValid.isUndefined(lang) ||
      isValid.isUndefined(page) ||
      isValid.isNotValidInteger(page) ||
      isValid.isUndefined(limit) ||
      isValid.isNotValidInteger(limit)
    ) {
      res.status(400).json({
        status: 'failed',
        message: '欄位未填寫正確',
  ...
  async delete_collection(req, res, next) {
    ...
    const result = await pool.query(
      'SELECT * FROM favorite WHERE user_id = $1 AND collection_id = $2',
    ...
    await pool.query('DELETE FROM favorite WHERE user_id = $1 AND collection_id = $2', [
      user_id,
      collection_id,
    ]);
    res.status(200).json({
