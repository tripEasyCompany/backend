...
//空值、為整數
//空值、為整數
    if(isValid.isEmpty(lang) || !isValid.isValidLang(lang) || isValid.isNotValidString(lang) ||
        isValid.isEmpty(page) || !isValid.isStringToInt(page) ||
        isValid.isEmpty(limit) || !isValid.isStringToInt(limit))
    {
        res.status(400).json({
            "status" : "failed",
            "message" : "欄位未填寫正確"
...
async function postCollection(req, res, next){
    ...
    if(isValid.isEmpty(tour_id) ||
        isValid.isNotValidString(tour_id)){ 
        res.status(400).json({
            "status" : "failed",
            "message" : "欄位未填寫正確"
        })
...
async function deleteCollection(req, res, next){
    ...
    if(isValid.isEmpty(favorite_id) ||
